...
import { NodeNav } from "@/components/layout/NodeNav";
import {
  getAdjacentNodeExercises,
  getNodeExercise,
  getNodeSection,
  nodeSections,
...
export async function generateStaticParams() {
  return nodeSections.flatMap((section) =>
...
export default async function NodeExercisePage({ params }: PageProps) {
  const { section: sectionSlug, slug: exerciseSlug } = await params;
  const section = getNodeSection(sectionSlug);
  const exercise = getNodeExercise(sectionSlug, exerciseSlug);
  ...
  const { prev, next } = getAdjacentNodeExercises(sectionSlug, exerciseSlug);
  ...
  return (
    <article className="space-y-6">
      ...
      <NodeNav sectionSlug={sectionSlug} prev={prev} next={next} />
    </article>
  );
}
