...
export function generateStaticParams() {
  return algoSections.map((section) => ({ section: section.slug }));
}

export default async function AlgoSectionPage({ params }: PageProps) {
              ...
              <ul className="mt-4 space-y-3">
                {subsection.exercises.map((exercise) => (
                  <li key={exercise.slug}>
                    <Link
                      href={algoHref(section.slug, exercise.slug)}
