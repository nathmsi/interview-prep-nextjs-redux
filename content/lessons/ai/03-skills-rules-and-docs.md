...
- Interview site: course content in content/lessons/, exercises in content/exercises/, UI in src/app/.
...
# Add lesson subject
1. Create content/lessons/<track>/...
...
```markdown
# Agent guide
- Lessons: content/lessons/<track>/*.md → registered in src/lib/lessons.ts
- Exercises: content/exercises/<track>/... → run with npm run test:exercises
- Subject pages: src/app/subjects/[subject]/page.tsx
- Tests: npm test (vitest), npm run test:e2e (playwright)
- Do not edit content/exercises in UI tasks unless asked
```
